...
async function main() {
  // -------------------------------------------------------------------------
  // Owner session — this server acts as one configured owner.
  // -------------------------------------------------------------------------
  const ownerEmail = (
    ...
    .toLowerCase();
  if (!ownerEmail) {
    console.error("Set MCP_OWNER_EMAIL or ALLOWED_EMAILS");
    process.exit(1);
  }
  ...
  console.error(`roadtrip MCP server ready (owner: ${session.email})`);
}
